...
Child IDs must come from environment variables, referenced by config.

Config file:
`skills/babyconnect-browser/.children.yaml`

Template (see file for full structure):
| Label | Child ID (env var) | Default Unit |
|------|---------------------|--------------|
| Child A | `${BABYCONNECT_CHILD_A_ID}` | ml |
| Child B | `${BABYCONNECT_CHILD_B_ID}` | oz |

**Default child:** `children.default` from `.children.yaml` (unless user specifies a different child)

**Runtime resolution (required):**
1. Load `.children.yaml`
2. For the chosen child key, read `id_env` from the config
3. Resolve the actual ID from `process.env[id_env]`
4. Inject that ID into any `evaluate` snippet before execution

If the env var is missing, stop and report: `ERR: Missing env var <id_env>`.

## Snippet Resolution (Required)

**New (2026-01-29):** Use the resolver script to preprocess snippets before execution.

```bash
# CLI usage
node skills/babyconnect-browser/resolve-children.js <snippet-path> <child-key>

# Example: Resolve bottle.js for Child A
node skills/babyconnect-browser/resolve-children.js snippets/bottle.js child_a

# Programmatic usage
const { resolveSnippet } = require('./skills/babyconnect-browser/resolve-children.js');
const snippet = resolveSnippet('snippets/bottle.js', 'child_a');
// snippet now has actual CHILD_ID injected, ready for browser.evaluate
```

**What the resolver does:**
1. Reads `.children.yaml` to get the `id_env` for the chosen child
2. Looks up the actual ID from `process.env[id_env]`
3. Substitutes `${BABYCONNECT_CHILD_A_ID}` and `${CHILD_ID}` in the template
4. Returns the resolved JavaScript ready for execution
...
dialog "Diaper" [ref=e2035]:
  - generic [ref=e2044]: Child B                  ← Child option
  - generic [ref=e2048]: Child A                  ← Child option (click this)
...
### Step 5: Click the child (default from config unless specified)
```
...
```
textbox: Child A had a BM and wet diaper (medium)
```
...
```
Child A had a BM and wet diaper (medium)
```
